LINTIGNOREDOT='spctesting/integration.+should not use dot imports'
...
integ-custom:
	go test -tags "integration" ./spctesting/integration/customizations/...

smoke-tests: get-deps-tests
	gucumber -go-tags "integration" ./spctesting/integration/smoke

performance: get-deps-tests
	SPC_TESTING_LOG_RESULTS=${log-detailed} SPC_TESTING_REGION=$(region) SPC_TESTING_DB_TABLE=$(table) gucumber -go-tags "integration" ./spctesting/performance
...
sandbox-test-go14:
	docker build -f ./spctesting/sandbox/Dockerfile.test.go1.4 -t "spc-sdk-go-1.4" .
	docker run -t spc-sdk-go-1.4

sandbox-test-go15:
	docker build -f ./spctesting/sandbox/Dockerfile.test.go1.5 -t "spc-sdk-go-1.5" .
	docker run -t spc-sdk-go-1.5

sandbox-test-go15-novendorexp:
	docker build -f ./spctesting/sandbox/Dockerfile.test.go1.5-novendorexp -t "spc-sdk-go-1.5-novendorexp" .
	docker run -t spc-sdk-go-1.5-novendorexp

sandbox-test-go16:
	docker build -f ./spctesting/sandbox/Dockerfile.test.go1.6 -t "spc-sdk-go-1.6" .
	docker run -t spc-sdk-go-1.6

sandbox-test-go17:
	docker build -f ./spctesting/sandbox/Dockerfile.test.go1.7 -t "spc-sdk-go-1.7" .
	docker run -t spc-sdk-go-1.7

sandbox-test-gotip:
	@echo "Run make update-spc-golang-tip, if this test fails because missing spc-golang:tip container"
	docker build -f ./spctesting/sandbox/Dockerfile.test.gotip -t "spc-sdk-go-tip" .
	docker run -t spc-sdk-go-tip

update-spc-golang-tip:
	docker build -f ./spctesting/sandbox/Dockerfile.golang-tip -t "spc-golang:tip" .
...
SDK_BASE_FOLDERS=$(shell ls -d */ | grep -v vendor | grep -v spcmigrate)
...
docs:
	...
	@if [ -n "$(SPC_DOC_GEN_TOOL)" ]; then echo "For internal use. Subject to change."; $(SPC_DOC_GEN_TOOL) `pwd`; fi
